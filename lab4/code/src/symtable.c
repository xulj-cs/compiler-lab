...
void initSymTable(){
	...
	ParaList_Ret para_ret = (ParaList_Ret) malloc(sizeof(struct FieldList_));
	para_ret->name = "return";
	para_ret->type = (Type) malloc(sizeof(struct Type_));
	para_ret->type->kind = BASIC;
	para_ret->type->basic = INT;
	para_ret->tail = NULL;
	insertSymTable("read",Function,para_ret,0);
		
	ParaList_Ret para_ret2 = (ParaList_Ret) malloc(sizeof(struct FieldList_));
	para_ret2->name = "return";
	para_ret2->type = para_ret->type;
	para_ret2->tail = (FieldList) malloc(sizeof(struct FieldList_));
	para_ret2->tail->name = NULL;
	para_ret2->tail->type = para_ret->type;
	para_ret2->tail->tail = NULL;
	insertSymTable("write",Function,para_ret2,0);
}
